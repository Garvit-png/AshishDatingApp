    ...
    >
      ...
      <div className="relative z-10 flex-1 flex flex-col lg:flex-row items-stretch max-w-[1440px] mx-auto w-full px-4 sm:px-10 lg:px-16 pt-6 pb-8 lg:pt-2 lg:pb-0">
        ...
        <div className="flex-none lg:w-[45%] flex flex-col justify-start py-4 lg:py-16 pr-0 lg:pr-12">
          ...
          {/* Badge */}
          <div className="inline-flex items-center gap-2 self-start mb-5 sm:mb-8" style={anim(0)}>
            <span
          ...
          {/* Heading — no whitespace-nowrap on mobile */}
          <h1
            className="font-black text-white tracking-tight mb-5 sm:mb-6"
            style={{
          ...
          >
            Build{" "}
            <span style={{ color: "#000", textShadow: "none" }}>Standards.</span>
            <br />
            Create{" "}
            <span style={{ color: "#000", textShadow: "none" }}>Attraction.</span>
            <br />
            Date With{" "}
            <span style={{ color: "#000", textShadow: "none" }}>Confidence.</span>
          </h1>

          {/* Subtitle */}
          <p
            className="text-sm sm:text-base italic mb-6 sm:mb-8 leading-relaxed max-w-md"
            style={{
          ...
          {/* Benefits list */}
          <div className="mb-6 sm:mb-10 flex flex-col" style={anim(240)}>
            {benefits.map((benefit, i) => (
              <div key={i}>
                <div className="flex items-center gap-3 py-2.5 sm:py-3">
                  <span
                    className="flex-none w-5 h-5 rounded-full flex items-center justify-center shrink-0"
                    style={{
                  ...
                  <span className="text-white text-xs sm:text-sm font-medium leading-snug" style={{ textShadow: "0 1px 4px rgba(0,0,0,0.3)" }}>
                    {benefit}
                  </span>
                </div>
                {i < benefits.length - 1 && (
          ...
          {/* CTA row */}
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-6 sm:mb-8" style={anim(340)}>
            <button
              onClick={onScrollToCourses}
              className="group relative inline-flex items-center gap-2 px-5 sm:px-7 py-3 sm:py-3.5 rounded-xl font-bold text-sm"
              style={{
                background: "#000",
                color: "#fff",
            ...
            >
              <span
                className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center pl-0.5"
                style={{ background: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.25)" }}
              >
                <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-current text-white">
          ...
          {/* Social proof */}
          <div className="flex items-center gap-3" style={anim(420)}>
            <div className="flex items-center">
              {avatarInitials.map((initial, i) => (
                <div
                  key={i}
                  className="w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-[10px] sm:text-[11px] font-bold text-white"
                  style={{
            ...
            <div>
              <p className="text-white text-xs sm:text-sm font-semibold leading-tight">1000+ Daily Hustlers</p>
        ...
        {/* ════ RIGHT SIDE ════ */}
        {/* On mobile: shown below left content, centered */}
        <div
          className="flex-1 flex flex-col items-center justify-center relative mt-8 lg:mt-0 lg:min-h-full"
          style={anim(200)}
        >
          {/* Quote ABOVE portrait */}
          <div
            className="w-full text-center pb-4 pt-2 relative z-10 px-4"
            style={{ fontFamily: "var(--font-cormorant), 'Cormorant Garamond', Georgia, serif" }}
          >
            <div className="flex items-center justify-center gap-4 mb-3">
              <div className="h-px flex-1 max-w-[60px] sm:max-w-[80px]" style={{ background: "linear-gradient(to right, transparent, rgba(255,255,255,0.5))" }} />
              <span className="text-[10px] font-bold tracking-[0.3em]" style={{ color: "rgba(255,255,255,0.6)", fontFamily: "var(--font-inter), sans-serif" }}>✦</span>
              <div className="h-px flex-1 max-w-[60px] sm:max-w-[80px]" style={{ background: "linear-gradient(to left, transparent, rgba(255,255,255,0.5))" }} />
            </div>
            <p
              className="text-white font-light tracking-[0.06em] leading-snug"
              style={{ fontSize: "clamp(1.2rem, 4vw, 2.2rem)", textShadow: "0 2px 8px rgba(0,0,0,0.4)" }}
            >
              Stop Chasing.
            </p>
            <p
              className="font-semibold tracking-[0.04em] mt-1"
              style={{
                fontSize: "clamp(1.4rem, 5vw, 2.8rem)",
                color: "#fff",
                textShadow: "0 0 24px rgba(0,0,0,0.5), 0 2px 8px rgba(0,0,0,0.4)",
              }}
            >
          ...
          >
            <div className="flex items-center justify-center gap-4 mt-2">
              <div className="h-px flex-1 max-w-[60px] sm:max-w-[80px]" style={{ background: "linear-gradient(to right, transparent, rgba(255,255,255,0.4))" }} />
              <span className="text-xs italic font-medium tracking-widest" style={{ color: "rgba(255,220,220,0.75)" }}>
                — Ashish Chhipa
              </span>
              <div className="h-px flex-1 max-w-[60px] sm:max-w-[80px]" style={{ background: "linear-gradient(to left, transparent, rgba(255,255,255,0.4))" }} />
            </div>
          </div>
        </div>
      </div>
